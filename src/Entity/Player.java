...
public class Player extends Entity {
	...
	// Add a field for the quest
	private Quest forgottenArtifactQuest;

	// Add a method to set the quest
	public void setForgottenArtifactQuest(Quest quest) {
	    this.forgottenArtifactQuest = quest;
	}
	...
	public void interactNPC(int i) {
	    ...
	    if (gp.keyH.enterPressed == true) {
	        ...
	        if (i != 999) {
	            ...
	            if (gp.npc[i] instanceof NPC_OldMan) {  // check if the NPC is the Old Man
	                NPC_OldMan oldMan = (NPC_OldMan) gp.npc[i];
	                oldMan.speak();  // This will now handle quest dialogues as well
	                oldMan.getQuest().checkCompletion(this);  // Check if quest conditions are met
	                if (oldMan.getQuest().isCompleted()) {
	                    // Give the player the quest rewards here
	                    gainXP(oldMan.getQuest().getRewardXP());
	                }
	            } else {
	                gp.npc[i].speak();
	            }
		...
		if (i != 999) {
			...
			if (gp.monster[i].invincible == false) {
				...
				if (gp.monster[i].life <= 0) {
					...
					checkLevelUp();
					 if ("Green Slime".equals(gp.monster[i].name)) {
				            slimesDefeated++;
				            gp.ui.addMessage("killed a slime");
					// check quest completion
					 if (quest != null && quest.checkCompletion(this)) {  // Pass 'this' as the player object
						    quest.completeQuest(this);
	                    gp.ui.addMessage("Quest '" + quest.getQuestName() + "' completed!");
	                    gp.ui.addMessage("You have gained " + quest.getRewardXP() + " XP!");
	                }
				}
			}
		}}
	...
	public int getSlimesDefeated() {
	    return slimesDefeated;
	}
	...
	public void checkLevelUp() {
	    while (exp >= nextLevelExp) { // Using while loop to handle cases where the player gains a lot of XP at once
	        exp -= nextLevelExp; // Subtract the required XP for leveling up
